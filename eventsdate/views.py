...
class JoinMeetingAPIView(APIView):
    def post(self, request, meeting_id):
        ...
        if existing_membership:
            ...
            meeting_member = models.MeetingMembers(user=user, meeting=meeting, text=text)
            meeting_member.save()
            message = 'Успешно присоединено'

        serializer = serializers.MeetingMembersSerializer(meeting_member) if 'meeting_member' in locals() else None
