...
class LikesToursListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        existing_like = models.LikesTours.objects.filter(
            tours=tour, user=user).first()
...
class LikesReviewsListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        existing_like = models.LikesReviews.objects.filter(
            reviews=review, user=user).first()
...
class JoinMeetingAPIView(APIView):
    def post(self, request, tour_id):
        ...
        existing_membertour = models.ToursMembers.objects.filter(
            user=user, tours=tour).first()

        if existing_membertour:
            existing_membertour.delete()
            message = 'Присоединение отменено'
        else:
            current_members_count = models.ToursMembers.objects.filter(
                tours=tour).count()

            if current_members_count >= tour.max_members:
                return Response({'error': 'Переполнено'}, status=status.HTTP_400_BAD_REQUEST)

            tours_member = models.ToursMembers(
                user=user, tours=tour, text=text)
            tours_member.save()
            message = 'Успешно присоединено'

        serializer = serializers.ToursMembersSerializer(
            tours_member) if 'tours_member' in locals() else None
